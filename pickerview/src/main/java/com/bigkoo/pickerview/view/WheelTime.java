...
import com.bigkoo.pickerview.utils.DateUtil;
...
public class WheelTime {
    ...
    public void setDayLabel(String dayLabel) {
        if (dayLabel != null) {
            ...
            this.dayLabel = view.getContext().getString(R.string.pickerview_day);
        }
    }
    ...
    private void setSolar(int year, final int month, int day, int h, int m, int s) {
        ...
        if (startYear == endYear && startMonth == endMonth) {
            if (list_big.contains(String.valueOf(month + 1))) {
                ...
                setSolarDayAdapter(year, month + 1, startDay, endDay);
            ...
            // 起始日期的天数控制
            if (list_big.contains(String.valueOf(month + 1))) {
                setSolarDayAdapter(year, month + 1, startDay, 31);
                //wv_day.setAdapter(new NumericWheelAdapter(startDay, 31));
            } else if (list_little.contains(String.valueOf(month + 1))) {
                setSolarDayAdapter(year, month + 1, startDay, 30);
                //wv_day.setAdapter(new NumericWheelAdapter(startDay, 30));
            } else {
                // 闰年
                if ((year % 4 == 0 && year % 100 != 0) || year % 400 == 0) {
                    setSolarDayAdapter(year, month + 1, startDay, 29);
                    //wv_day.setAdapter(new NumericWheelAdapter(startDay, 29));
                } else {
                    setSolarDayAdapter(year, month + 1, startDay, 28);
            ...
            if (list_big.contains(String.valueOf(month + 1))) {
                ...
                setSolarDayAdapter(year, month + 1, 1, endDay);
            ...
            // 判断大小月及是否闰年,用来确定"日"的数据
            if (list_big.contains(String.valueOf(month + 1))) {
                setSolarDayAdapter(year, month + 1, 1, 31);
                //wv_day.setAdapter(new NumericWheelAdapter(1, 31));
            } else if (list_little.contains(String.valueOf(month + 1))) {
                setSolarDayAdapter(year, month + 1, 1, 30);
                // wv_day.setAdapter(new NumericWheelAdapter(1, 30));
            } else {
                // 闰年
                if ((year % 4 == 0 && year % 100 != 0) || year % 400 == 0) {
                    setSolarDayAdapter(year, month + 1, 1, 29);
                    //wv_day.setAdapter(new NumericWheelAdapter(1, 29));
                } else {
                    setSolarDayAdapter(year, month + 1, 1, 28);
    ...
    private void setSolarDayAdapter(int year, int month, int startDay, int endDay) {
        if (dayIsShowWeek) {
            wv_day.setAdapter(new ArrayWheelAdapter(getSolarDayWeekList(year, month, startDay, endDay)));
        } else {
            wv_day.setAdapter(new NumericWheelAdapter(startDay, endDay));
        }
    }
     ...
     * @return
     */
    private List<String> getSolarDayWeekList(int year, int month, int startDay, int endDay) {
        List<String> dayWeekList = new ArrayList<>();
        for (int i = startDay; i <= endDay; i++) {
            String week = DateUtil.getWeek(year + "-" + month + "-" + i);
            dayWeekList.add(i + dayLabel + week);
        }
        return dayWeekList;
    }
...
//        int maxItem;
        if (list_big.contains(String.valueOf(monthNum))) {
            ...
            setSolarDayAdapter(year_num, monthNum, startD, endD);
